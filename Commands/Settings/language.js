...
module.exports = {
	...
	async execute(interaction) {
		...
		languageSchema.findOne({
			...
			const fin_language_change_alreadyset = localisation.language_change_alreadyset.replace('${language}', `\`${language}\``)
			if (data.Language == language) return interaction.reply(fin_language_change_alreadyset)

			data.Language = language
			await data.save()

			const fin_language_changeconfirm = localisation.language_changeconfirm.replace('${language}', `\`${language}\``)
