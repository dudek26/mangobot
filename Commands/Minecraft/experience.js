...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName("level")
				.setDescription(
					"Calculate what level you have to get to have your target experience."
				)
				.addNumberOption((option) =>
					option
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName("difference")
				.setDescription(
					"Calculate how much experience is needed to get your target level."
				)
				.addNumberOption((option) =>
					option
						.setName("current_level")
	...
	async execute(interaction) {
		...
		switch (true) {
			...
			case current_level <= 16:
				var current_experience =
					current_level * current_level + 6 * current_level;
				break;
			case current_level <= 31 && current_level > 16:
				var current_experience =
					2.5 * current_level * current_level - 40.5 * current_level + 361;
				break;
			case current_level > 31:
				var current_experience =
					4.5 * current_level * current_level - 162.5 * current_level + 2221;
				break;
			default:
		...
		switch (interaction.options.getSubcommand()) {
			case "current":
				const fin_experience_current = localisation.experience_current.replace(
					"${current_experience}",
					`\`${current_experience}\``
				);
			...
			case "level":
				var target_experience =
					interaction.options.getNumber("target_experience");
				switch (true) {
					...
					case target_experience <= 1507 && target_experience > 352:
						var target_level =
							81 / 10 + Math.sqrt((2 / 5) * (target_experience - 7839 / 40));
						break;
					case target_experience > 1507:
						var target_level =
							325 / 18 + Math.sqrt((2 / 9) * (target_experience - 54215 / 72));
						break;
					default:
						interaction.reply(localisation.error);
						return;
				}
				const fin_experience_gettolvl =
					localisation.experience_gettolvl.replace(
						"${target_level}",
						`\`${target_level.toFixed(2)}\``
					);
			...
			case `difference`:
				...
				switch (true) {
					...
					case target_level <= 16:
						var target_experience =
							target_level * target_level + 6 * target_level;
						break;
					case current_level <= 31 && target_level > 16:
						var target_experience =
							2.5 * target_level * target_level - 40.5 * target_level + 361;
						break;
					case current_level > 31:
						var target_experience =
							4.5 * target_level * target_level - 162.5 * target_level + 2221;
						break;
					default:
						interaction.reply(localisation.error);
						return;
				...
				switch (true) {
					case experience_difference == 0:
						...
						break;
					case experience_difference > 0:
						const fin_experience_difference_more =
							localisation.experience_difference_more.replace(
						...
						break;
					case experience_difference < 0:
						const fin_experience_difference_less =
							localisation.experience_difference_less.replace(
								"${experience_difference}",
				...
				break;
			case "addition":
				var additional_experience = interaction.options.getNumber(
				...
				const fin_experience_sum = localisation.experience_sum.replace(
					"${target_level}",
					`\`${target_level.toFixed(2)}\``
				);
